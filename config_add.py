...
if __name__ == "__main__":
    ...
    if not SO_CONFIG:
       exit()
#### find all cluster devices

    (s,cluster_list)=sdk.get(uri='/axapi/v3/scaleout/cluster/'+str(so_cluster)+'/cluster-devices/')

#    cluster_list['cluster-devices']['device-id-list'].append({'action': 'enable','device-id': ID, 'ip': ADDRESS})
#
#
#    (s,r)=sdk.post(uri='/axapi/v3/scaleout/cluster/'+str(so_cluster)+'/cluster-devices/', payload=cluster_list)
#    if s != 200:
#       print "ERROR:", s, r
    commands=[];
    commands.append("scaleout "+str(so_cluster))
    commands.append("cluster-devices")
    commands.append("device-id "+str(ID))
    commands.append("ip "+str(ADDRESS))

    cluster_config = {
      "commandList": commands,
    }

    print "configuratio to add: "
...
#    (s,r) = sdk.get(uri='/axapi/v3/scaleout/cluster')
#    print r['cluster']['local-device']['id']
#    print (r['cluster-list'][0]['cluster-devices'])
